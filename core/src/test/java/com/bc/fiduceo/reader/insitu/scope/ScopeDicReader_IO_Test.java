...
/**
 * Integration tests for ScopeDicReader using real data files.
 * <p>
 * Tests the complete reader functionality with actual SCOPE DIC files.
 * Follows the same pattern as ScopeFco2Reader_IO_Test.
 */
@RunWith(IOTestRunner.class)
public class ScopeDicReader_IO_Test {

    private ScopeDicReader reader;

    @Before
    public void setUp() {
        reader = new ScopeDicReader(new GeometryFactory(GeometryFactory.Type.S2));
    }

    @Test
    public void testReadAcquisitionInfo() throws IOException {
        final File testFile = getDicTestFile();
    ...
    public void testGetVariables() throws Exception {
        ...
        try {
            ...
            boolean hasInSituDic = false;
            boolean hasInSituDicStd = false;
            boolean hasUExpFnnUDic = false;
            ...
            for (Variable var : variables) {
                ...
                if ("in_situ_DIC".equals(name)) hasInSituDic = true;
                if ("in_situ_DIC_std".equals(name)) hasInSituDicStd = true;
                if ("UExP_FNN_U_DIC".equals(name)) hasUExpFnnUDic = true;
            ...
            assertTrue("Should have in_situ_DIC variable", hasInSituDic);
            assertTrue("Should have in_situ_DIC_std variable", hasInSituDicStd);
            assertTrue("Should have UExP_FNN_U_DIC variable", hasUExpFnnUDic);
    ...
    @Test
    public void testReadScaled_InSituDic() throws Exception {
        final File testFile = getDicTestFile();

        try {
            reader.open(testFile);
            final Array array = reader.readScaled(0, 0, new Interval(1, 1), "in_situ_DIC");

            assertNotNull("Should read in_situ_DIC variable", array);
    ...
    public void testExtractYearMonthDayFromFilename() {
        int[] ymd = reader.extractYearMonthDayFromFilename("SCOPE_WP21_DIC_1985_2022.txt");
    ...
    public void testGetRegEx() {
        ...
        assertEquals("SCOPE_WP21.*DIC.*\\.txt", regex);

        // Test pattern matching
        assertTrue("SCOPE_WP21_DIC_1985_2022.txt".matches(regex));
        assertTrue("SCOPE_WP21_DIC_something_1985_2022.txt".matches(regex));
        assertFalse("SCOPE_WP21_fCO2_1985_2023.txt".matches(regex));
        assertFalse("random_file.txt".matches(regex));
    }

    @Test
    public void testParseLine_ValidLine() {
        final String validLine = "477273600 57.500 -29.500 2150.500 15.300 2152.100 22.500 0.000";
        final DicRecord record = reader.parseLine(validLine);
        ...
        assertEquals(2150.500f, record.inSituDic, 0.001f);
        assertEquals(15.300f, record.inSituDicStd, 0.001f);
        assertEquals(2152.100f, record.uExpFnnUDic, 0.001f);
        assertEquals(22.500f, record.uExpFnnUUncertainty, 0.001f);
    ...
    public void testParseLine_InvalidLineTooFewTokens() {
        ...
        final DicRecord record = reader.parseLine(invalidLine);
    ...
    private static File getDicTestFile() throws IOException {
        final String relativePath = TestUtil.assembleFileSystemPath(
                new String[]{"insitu", "wp21", "SCOPE_WP21_DIC_1985_2022.txt"}, false
        );
