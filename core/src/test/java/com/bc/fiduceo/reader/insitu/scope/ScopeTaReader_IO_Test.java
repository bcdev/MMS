...
/**
 * Integration tests for ScopeTaReader using real data files.
 * <p>
 * Tests the complete reader functionality with actual SCOPE total alkalinity files.
 * Follows the same pattern as ScopepHReader_IO_Test.
 */
@RunWith(IOTestRunner.class)
public class ScopeTaReader_IO_Test {

    private ScopeTaReader reader;

    @Before
    public void setUp() {
        reader = new ScopeTaReader(new GeometryFactory(GeometryFactory.Type.S2));
    }

    @Test
    public void testReadAcquisitionInfo() throws IOException {
        final File testFile = getTaTestFile();
    ...
    public void testGetVariables() throws Exception {
        ...
        try {
            ...
            boolean hasInSituTa = false;
            boolean hasInSituTaStd = false;
            boolean hasUExpFnnUTa = false;
            ...
            for (Variable var : variables) {
                ...
                if ("in_situ_TA".equals(name)) hasInSituTa = true;
                if ("in_situ_TA_std".equals(name)) hasInSituTaStd = true;
                if ("UExP_FNN_U_TA".equals(name)) hasUExpFnnUTa = true;
            ...
            assertTrue("Should have in_situ_TA variable", hasInSituTa);
            assertTrue("Should have in_situ_TA_std variable", hasInSituTaStd);
            assertTrue("Should have UExP_FNN_U_TA variable", hasUExpFnnUTa);
    ...
    @Test
    public void testReadScaled_InSituTa() throws Exception {
        final File testFile = getTaTestFile();

        try {
            reader.open(testFile);
            final Array array = reader.readScaled(0, 0, new Interval(1, 1), "in_situ_TA");

            assertNotNull("Should read in_situ_TA variable", array);
    ...
    public void testExtractYearMonthDayFromFilename() {
        int[] ymd = reader.extractYearMonthDayFromFilename("SCOPE_WP21_TA_1972_2023.txt");

        assertEquals(3, ymd.length);
        assertEquals(1972, ymd[0]);
    ...
    public void testGetRegEx() {
        ...
        assertEquals("SCOPE_WP21.*TA.*\\.txt", regex);

        // Test pattern matching
        assertTrue("SCOPE_WP21_TA_1972_2023.txt".matches(regex));
    ...
    public void testParseLine_ValidLine() {
        final String validLine = "90374400 -33.000 -21.008 2421.000 0.000 2431.094 25.752 0.000";
        final TaRecord record = reader.parseLine(validLine);

        assertNotNull("Should parse valid line", record);
        assertEquals(90374400, record.utc);
        assertEquals(-33.000f, record.longitude, 0.001f);
        assertEquals(-21.008f, record.latitude, 0.001f);
        assertEquals(2421.000f, record.inSituTa, 0.001f);
        assertEquals(0.000f, record.inSituTaStd, 0.001f);
        assertEquals(2431.094f, record.uExpFnnUTa, 0.001f);
        assertEquals(25.752f, record.uExpFnnUUncertainty, 0.001f);
        assertEquals(0.000f, record.region, 0.001f);
    }

    @Test
    public void testParseLine_InvalidLineTooFewTokens() {
        final String invalidLine = "90374400 -33.000 -21.008";
        final TaRecord record = reader.parseLine(invalidLine);
    ...
    private static File getTaTestFile() throws IOException {
        final String relativePath = TestUtil.assembleFileSystemPath(
                new String[]{"insitu", "wp21", "SCOPE_WP21_TA_1972_2023.txt"}, false
        );
